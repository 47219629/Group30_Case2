...
fig2, ax1 = plt.subplots(figsize=(6.8, 5))
...
plt.axvline(x=float(stock_data['Close'].iloc[len(stock_data['Close'])-1].item()), color='r', linestyle='--', label='Spot: A${:.2f}'.format(stock_data['Close'].iloc[len(stock_data['Close'])-1].item())) # Add legend to display the label
plt.axvline(x=float(Target_Price), color='g', linestyle='--', label='Target: A${:.2f}'.format(Target_Price))
plt.legend(fontsize=18,loc="center right",bbox_to_anchor=(1.04,-0.25))  # Add legend to display the label
